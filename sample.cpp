...
// auto coll_names = db.list_collection_names();

// std::cout << "CollectionS:" << std::endl;

// for(auto& name : coll_names) {
...
void create_json_schema_file(bsoncxx::document::view_or_value kms_providers,
  ...
  auto json_schema = document{}
                     << "properties" << open_document << "encryptedField"
...
int main2(int, char **) {
  ...
  auto expression = document{} << "keyAltNames" << open_document << "$exists"
  ...
  create_json_schema_file(kms_providers.view(), {"keyvault", "datakeys"},
                          &key_vault_client);
...
int main(int, char **) {
  try {
    ...
    std::cout << "caught exception222" << std::endl;
